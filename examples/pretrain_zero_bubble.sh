...
options=" \
  ...
  --max-position-embeddings $(($SEQ_LENGTH * 2)) \
  --micro-batch-size $MICRO_BATCH_SIZE \
  --global-batch-size $GLOBAL_BATCH_SIZE \
  --train-samples $SAMPLES \
  --lr-decay-samples $(($SAMPLES - 183105)) \
...
if [ ! -z "$ZERO_BUBBLE_V_SCHEDULE" ]; then
  ...
  if [ ! -z "$ZERO_BUBBLE_V_SCHEDULE_MEM_SETUP" ]; then
    options="$options --zero-bubble-v-schedule-mem-setup $ZERO_BUBBLE_V_SCHEDULE_MEM_SETUP"
  fi
fi
