...
class WeightGradStore:
    def __init__(self):
        ...
        self.weight_grad_queue = [queue.Queue() for _ in range(num_vpp)]

    def put(self, total_input, grad_output, weight, func):
        # func(total_input, grad_output, weight.main_grad)
        self.cache.append((total_input, grad_output, weight, func))

    def flush(self, chunk=0):
        self.weight_grad_queue[chunk].put(self.cache)
        self.cache = []

    def pop(self, chunk=0):
        if self.weight_grad_queue[chunk].qsize() > 0:
            stored_grads = self.weight_grad_queue[chunk].get()
            for total_input, grad_output, weight, func in stored_grads:
                func(total_input, grad_output, weight.main_grad)
        else:
            raise IndexError("Pop empty queue.")

    def clear(self, model, chunk=0):
        weight_grad_tasks = []
        while self.weight_grad_queue[chunk].qsize() > 0:
            stored_grads = self.weight_grad_queue[chunk].get()
